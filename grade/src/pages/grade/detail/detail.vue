<template>
    ...
    <!-- 
    ############################################
                此页面已暂停使用 -> components.gradeDeatil.vue
    ############################################
    -->
...
export default {
  ...
  methods:{
    ...
    getGradeItemList () {
      ...
      return db.collection('paperSubject').where({id:this.gradeId}).get()
    },
    getTouristGrade () {
      const db = wx.cloud.database()
      ...
      .then(res => {
        ...
        console.log(JSON.parse(JSON.stringify({data: res.data, avgData:avgData})))
